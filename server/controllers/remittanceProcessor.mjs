...
export const registerCustomer = async (sendingAnchorDomain, jwtToken, customerInfo) => {
    try {
      ...
      const response = await fetch(`${kycServer}/customer`, {
        ...
        body: JSON.stringify(customerInfo), // Customer KYC data
      });
  
      const result = await response.json();
      return result.id; // Return the customer ID
    } catch (error) {
      console.error('Error resolving Stellar TOML or registering customer:', error);
      throw new Error(`Failed to resolve TOML or register customer: ${error.message}`);
    }
  };
